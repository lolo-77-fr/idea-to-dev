...
- **Doc status**: the six step docs (BRAINSTORM → CDC) carry `Statut : brouillon | validé | à revoir — [cause]`; resume detection uses it (not mere file existence), and the change-propagation rule sets downstream docs to `à revoir`. Rules in the orchestrator's "Statut des docs"; each step skill has a "Statut du document" section (checked by `scripts/check.sh`).
