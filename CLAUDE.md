...
- **User tests (`TESTS.md`)**: generated by `dev-loop` at the end of each brique (after proposing the brique recette, never blocking), points `U-01` with Préparer / Faire / Attendu / Résultat [OK/KO/Pas pu tester] / Remarque; the agent reads the filled file back and routes each feedback (bug → `A-XX` + corrective task, change request → propagation rule, question → answer), appending "Suite" lines without editing the user's text. `recette` complète adds a "Parcours complet" section and its verdict waits for it.
