...
- **Mode rapide** (projet très simple — cf. orchestrateur `idea-to-dev` ; sur proposition de l'orchestrateur ou demande de l'utilisateur) : rédiger le doc complet d'un coup en affichant en tête les hypothèses retenues, poser au plus 3 questions — regroupées dans un seul message, avec recommandation — uniquement sur les points à risque, et faire valider le doc en une fois plutôt que section par section. Garder le challenge sur le point le plus douteux de l'idée ; `brainstorm` et `product-brief` peuvent être produits dans le même échange.
